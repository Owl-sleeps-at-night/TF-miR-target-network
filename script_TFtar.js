...
let filteredTFTargetStep1 = [];
let filteredTFTargetStep2 = [];
// 设置默认激活按钮
// 包裹逻辑到一个初始化函数中
    function initializeScriptA() {
        console.log('scriptA initialized.');
        // 检查当前激活的模块
...
async function  runiPredictLogic(){
    ...
    if(typeof window.handleModechange==='function'){
        const modeSelect = document.getElementById("mode_TFtar");
        modeSelect.removeEventListener("change", window.handleModechange);
    };
    // ipredict初始参数
...
function filterGenefamily() {
    ...
    // 获取用户选择的模式
    let mode = document.getElementById('mode_TFtar').value;
    ...
    filteredTFTargetStep1 = filteredTFTarget;
    // 动态生成勾选框
...
function filterData() {
    ...
    console.log('filteredTFTargetStep1:', filteredTFTargetStep1);
    ...
    if (filteredTFTarget.length === 0) {
        alert('No data available after filtering. Please adjust your criteria.');
        displayGenefamilyTable();
        return;
    }

    // 暂存筛选结果以供后续使用
    filteredTFTargetStep2 = filteredTFTarget;
...
function applyGeneFamilyFilter() {
    ...
    let filteredTFTarget = filteredTFTargetStep2;
    // 更新target的表格
    updateTargetTable(filteredTFTarget);
...
document.getElementById("filter2Button").addEventListener("click", () => {
    ...
    document.querySelector('#search-node-button_TFtar').removeEventListener('click',window.searchbuttonClickhandler1);
...
function updateTargetTable(filteredTFTarget) {
    console.log('UpdateTartable TFtar!!');
...
function drawDisjointForceDirectedGraph(filteredTFTarget) {
    ...
    window.searchbuttonClickhandler1=()=>{
        ...
        if (matchedNode.length>0) {
            ...
            highlightMultipleNodes(matchedNode);
    ...
    document.querySelector("#search-node-button_TFtar").addEventListener('click',window.searchbuttonClickhandler1);
