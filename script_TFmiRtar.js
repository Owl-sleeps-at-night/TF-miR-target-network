...
let filteredMiRTargetStep1 = [];
let filteredMiRTargetStep2 = [];
// 设置默认激活按钮
// 包裹逻辑到一个初始化函数中
    function initializeScript3() {
        console.log('script3 initialized.');
        // 检查当前激活的模块
        let activeModule = activeNADCButton;
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeScript3);
} else {
    initializeScript3();
}
...
// 下载文件的事件监听器
    document.getElementById("download1").addEventListener("click", handleDownload_miRtar);
    function handleDownload_miRtar() {
        downloadDataAsTabDelimited(globalMiRTargetData, 'miRNA-Target-Data.txt');
    }
    document.getElementById("download2").addEventListener("click", handleDownload_tfmiR);
    function handleDownload_tfmiR() {
    ...
    // 定义回调函数
    window.nadcButtonClickHandler=function () {
        console.log('TFmiRtar 触发器被触发！');
        const buttonId = activeNADCButton;
        handleNADCButtonClick(buttonId);
    }
    // 添加事件监听器
    document.querySelectorAll('.nadc-button').forEach(button => {
        button.addEventListener('click', nadcButtonClickHandler);
        console.log('TFmiRtar触发器被添加！')
    });

// 添加按钮点击事件监听器
//     let nadcButtonListener = [];
...
async function  runiPredictLogic(){
    ...
    // 如果已经为change按钮绑定了事件监听器，移除这个事件监听器
    if(typeof window.handleModechange1==='function'){
        const modeSelect = document.getElementById("mode_TFmiRtar");
        modeSelect.removeEventListener("change", window.handleModechange1);
    };
    // ipredict初始参数
    ...
    const modeSelect = document.getElementById("mode_TFmiRtar");
    // 监听下拉菜单的 change 事件
     window.handleModechange1=function(){
    ...
    modeSelect.addEventListener("change", window.handleModechange1);
...
async function loadData() {
    try {
        ...
        // 与TF-target部分统一格式，使用 | 分隔多个mRNA subcellular location和miRNA family
...
function filterSubcellularLocations() {
    // waitingFunction();
    // 获取用户选择的筛选条件
    const ipre_cellCondition = document.getElementById('cell_condition').value;
    const ipre_artCondition = ART_condition;
    const ipre_cancerCondition = document.getElementById('cancer_condition').value;
    console.log('art:', ipre_artCondition);
    console.log('cell:', ipre_cellCondition);
    console.log('cancer:', ipre_cancerCondition);
    // 筛选 miRNA-target 数据
    let filteredMiRTarget = miRTargetData.filter(item => {
        return (
            (ipre_cellCondition ? item.cell_condition.includes(ipre_cellCondition) : true) &&
            (ipre_artCondition ? item.ART_condition.includes(ipre_artCondition) : true) &&
            (ipre_cancerCondition ? item.cancer_condition.includes(ipre_cancerCondition) : true)
        );
    });
    if(filteredMiRTarget.length === 0){
        alert('No miRNAs known to regulate the given features, please select another NADC.');
        document.getElementById('cell_condition').value='CD4';
        document.getElementById("ART_yes").checked=true;
        document.getElementById("ART_no").checked=false;
        ART_condition='ART'
    ...
    // 获取用户选择的模式
    let mode = document.getElementById('mode_TFmiRtar').value;
    ...
    filteredMiRTargetStep1 = filteredMiRTarget;
    // 动态生成勾选框
...
function filterSubcellularLocations1() {
    // waitingFunction();
    // 获取用户选择的筛选条件
    const dnadc_filterCondition = document.getElementById('dnadc_filter').value;
    const dnadc_artCondition = ART_condition_dNADC;
    const dnadc_cancerCondition = document.getElementById('dnadc_cancer_condition').value;
    console.log('dNADC - Selected Conditions:', { dnadc_filterCondition, dnadc_artCondition, dnadc_cancerCondition });
    // 筛选 miRNA-target 数据
    let filteredMiRTarget = miRTargetData.filter(item => {
        const matchesFilter = dnadc_filterCondition === 'A'
            ? ['A', 'L'].includes(item.filter_condition[0])
            : item.filter_condition.includes(dnadc_filterCondition);
        return (
            (dnadc_artCondition ? item.ART_condition.includes(dnadc_artCondition) : true) &&
            (dnadc_cancerCondition ? item.cancer_condition.includes(dnadc_cancerCondition) : true) &&
            matchesFilter
        );
    });
    if(filteredMiRTarget.length === 0){
...
if (filteredMiRTarget.length === 0) {
    ...
    // return;
    // 清除之前的数据
    filteredMiRTargetStep1 = []; // 确保清除之前的筛选数据
    filterSubcellularLocations1();
    return;
    // // 手动触发 change 事件
...
function filterData() {
    ...
    // 获取第一步筛选结果
    let filteredMiRTarget =  filteredMiRTargetStep1.filter(item => {
        if (item.Subcellular_location) {
            const locations = item.Subcellular_location[0].split(',');
    ...
    // 暂存筛选结果以供后续使用
    filteredMiRTargetStep2 = filteredMiRTarget;
    // 在页面上生成勾选框
...
function applyMiRFamilyFilter() {
    ...
    let filteredMiRTarget = filteredMiRTargetStep2.filter(item =>
...
document.getElementById("filter2Button").addEventListener("click", () => {
    ...
    document.querySelector('#search-node-button_TFmiRtar').removeEventListener('click',window.searchbuttonClickhandler);
...
function updateTargetTable(filteredMiRTarget) {
    console.log('UpdateTartable miRtar!!');
    const loadingPlaceholder = document.getElementById("loading-placeholder");
    document.getElementById("target-table-footer").style.display = "block";
    document.getElementById("target-table-footer_TF-target").style.display = "none";
    document.getElementById("target-table").style.display = "table";
    document.getElementById("target-table_TF-target").style.display = "none";
    // 隐藏加载图标
    ...
    uniqueTargets.forEach(item => {
        ...
        const locationCell = document.createElement("td");
        // 处理 Subcellular location 字符串
        if (item.subcellularLocation !== "NA") {
            // 使用正则替换左右都不为空格的逗号为管道符
            item.subcellularLocation = item.subcellularLocation.replace(/(?<=\S),(?=\S)/g, '|');
        }
...
function drawDisjointForceDirectedGraph(filteredMiRTarget, filteredTFWithMultipleMiRNAs) {
    ...
    filteredMiRTarget.forEach(item => {
        ...
        const experiments = (item.expreiment && item.expreiment.length > 0)
            // ? item.expreiment[0].split(',').map(e => e.trim()) // 如果存在，拆分实验条件并去掉多余空格
                ? item.expreiment.map(e => e.trim()) // 如果存在，拆分实验条件并去掉多余空格
    ...
    // 5. 设置初始缩放比例
    const initialScale = 0.5; // 设置初始缩放比例
    const initialTranslate = [width * 0.5, height * 0.55]; // 设置初始平移，使网络能够显示更多节点

    // 应用初始变换（缩放 + 平移）
    const initialTransform = d3.zoomIdentity
        .scale(initialScale) // 设置缩放比例
        .translate(initialTranslate[0], initialTranslate[1]); // 设置平移位置
    ...
    const simulation = d3.forceSimulation(nodes)
        .force("link", d3.forceLink(links).id(d => d.id).distance(200)) // 链接力
    ...
    const link = g.append("g")
        ...
        .attr("d", d => calculatePath(d)); // 使用 calculatePath 函数计算路径
    // 动态调整箭头位置函数
    ...
    function calculatePath(d, links) {
        ...
        const offset = (index - (sameEdgeGroup.length - 1) / 2) * 15; // 偏移量
        ...
        // 曲率调整，避免多条边重叠
        const curvature = 0.4; // 调整曲率的强度，较大值会让曲线更弯曲
        // 计算路径
        const dr = distance * (1 + Math.abs(offset) * curvature); // 根据偏移量调整曲率
        const angle = Math.atan2(dy, dx); // 计算两点之间的角度
        const arc = `M${d.source.x},${d.source.y}A${dr},${dr} 0 0,1 ${d.target.x},${d.target.y}`;
        return arc;
    ...
    window.searchbuttonClickhandler=()=>{
        console.log('搜索逻辑运行！')
        const searchInput = document.querySelector("#node-search-input").value;
        if (!searchInput) {
        ...
        // 在当前图中查找匹配的节点
        const matchedNode =  nodes.filter(node => node.id.replace(/_(target|TF|miRNA)$/, "").toLowerCase() === searchInput.toLowerCase().trim());
        if (matchedNode.length>0) {
            console.log("Nodes found:", matchedNode);
            highlightMultipleNodes(matchedNode); // 调用高亮节点逻辑
        } else if (matchedNode.length===0) {
            alert(`Node "${searchInput}" not found in the network.`);
            return;
        }
    };
    // 添加事件监听器
    document.querySelector("#search-node-button_TFmiRtar").addEventListener("click", window.searchbuttonClickhandler);
}
// document.querySelector("#search-node-button_TFmiRtar").addEventListener("click", () => {
    //     console.log('搜索逻辑运行！')
    //     const searchInput = document.querySelector("#node-search-input").value;
    //     if (!searchInput) {
    //         alert("Please enter a node name to search.");
    ...
    //     } else if (matchedNode.length===0) {
    //         alert(`Node "${searchInput}" not found in the network.`);
    //         document.querySelector("#node-search-input").value="";
    //         // drawDisjointForceDirectedGraph(globalMiRTargetData, globalTFMiRNAData);
    //         return;
    //     }
    // });
    ...
    if (data.length > 0) {
        ...
        data.forEach(item => {
            ...
            const row = headers.map(key => {
                let value = item[key] || ''; // 如果没有值，默认空字符串
